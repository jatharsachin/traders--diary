...
- **Dhan API Browser CORS Proxy Engine**: Configured local and internal proxy routing `/dhan-api` in `vite.config.ts` and `src/utils/dhanClient.ts` to seamlessly forward `access-token` and `client-id` headers without browser CORS preflight blocking or header stripping, providing instant 1-click Dhan authentication.
- **Dhan Direct Broker Connect (Pure Read-Only Auto-Sync)**: Added official DhanHQ API integration via `src/utils/dhanClient.ts` with pure `GET` read-only tradebook and fund limits fetch (zero order execution capability), dedicated isolated modal `src/components/DhanConnectModal.tsx`, interactive preview reconciliation table with duplicate detection, automatic Indian tax & brokerage calculation, and top-bar/account sync entry points.
