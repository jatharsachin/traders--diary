...
export function TradeLogger({ isOpen, onClose, editTradeId, activeAccountId }: TradeLoggerProps) {
  ...
  const [editingLeg1Id, setEditingLeg1Id] = useState<string | null>(null);
  const [editingLeg2Id, setEditingLeg2Id] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (editTradeId) {
      ...
      if (existing) {
        // Detect if this trade is part of a 2-leg spread / hedge
        const isSpreadLeg2 = existing.tags?.includes('#spread_leg2') || existing.tags?.includes('#hedge_leg') || (existing.notes && existing.notes.includes('Hedge Leg for'));
        const isSpreadLeg1 = existing.tags?.includes('#spread_leg1');
        const isHedged = existing.tags?.includes('#hedged');

        let leg1: typeof existing | undefined = undefined;
        let leg2: typeof existing | undefined = undefined;
        ...
        if (isSpreadLeg2) {
          ...
          leg1 = trades.find(t => 
            ...
            t.entryTime === existing.entryTime &&
            (t.brokerAccountId === existing.brokerAccountId || t.broker === existing.broker) &&
            (t.tags?.includes('#spread_leg1') || t.tags?.includes('#hedged') || t.strategy === existing.strategy)
          );
        } else if (isSpreadLeg1) {
          leg1 = existing;
          leg2 = trades.find(t => 
            t.id !== existing.id &&
            t.date === existing.date &&
            t.entryTime === existing.entryTime &&
            (t.brokerAccountId === existing.brokerAccountId || t.broker === existing.broker) &&
            (t.tags?.includes('#spread_leg2') || t.tags?.includes('#hedge_leg') || (t.notes && t.notes.includes('Hedge Leg')))
          );
        } else if (isHedged) {
          const sibling = trades.find(t => 
            t.id !== existing.id &&
            t.date === existing.date &&
            t.entryTime === existing.entryTime &&
            (t.brokerAccountId === existing.brokerAccountId || t.broker === existing.broker) &&
          ...
          if (sibling) {
            if (existing.action === 'SELL' || existing.tags?.includes('#spread_leg1')) {
              leg1 = existing;
              leg2 = sibling;
            } else {
              leg1 = sibling;
        ...
        if (leg1 && leg2) {
          // Paired hedge spread found! Load both legs into the form.
          setEditingLeg1Id(leg1.id);
          ...
          const initialManualBrokerage = leg1.manualBrokerage !== undefined && leg1.manualBrokerage > 0
            ? leg1.manualBrokerage
            : (leg1.brokerage !== undefined ? leg1.brokerage : 0);

          const initialManualTaxes = leg1.manualTaxes !== undefined && leg1.manualTaxes > 0
            ? leg1.manualTaxes
            : (leg1.taxes !== undefined ? leg1.taxes : 0);

          const cleanNotes = (leg1.notes || '').replace(/\(Hedge Leg.*?\)/gi, '').trim();
          const cleanTags = (leg1.tags || []).filter(t => !['#spread_leg1', '#spread_leg2', '#hedge_leg', '#hedged'].includes(t.toLowerCase()));

          setSelectedMistakes(getTradeMistakes(leg1));
          setFormData({
            date: leg1.date,
            entryTime: leg1.entryTime,
            exitTime: leg1.exitTime,
            exitDate: leg1.exitDate || leg1.date,
            segment: leg1.segment,
            product: leg1.product,
            action: leg1.action,
            symbol: leg1.symbol,
            qty: leg1.qty,
            entryPrice: leg1.entryPrice,
            exitPrice: leg1.exitPrice,
            slippagePoints: leg1.slippagePoints,
            stopLoss: leg1.stopLoss,
            target: leg1.target,
            strategy: leg1.strategy,
            rulesFollowed: leg1.rulesFollowed || [],
            emotion: leg1.emotion,
            mistake: leg1.mistake,
            notes: cleanNotes,
            strikePrice: leg1.strikePrice || 0,
            optionType: leg1.optionType || 'None',
            setupType: leg1.setupType || 'None',
            useManualCharges: leg1.useManualCharges || false,
            manualBrokerage: initialManualBrokerage,
            manualTaxes: initialManualTaxes,
            holdingType: leg1.holdingType || 'Short Term',
            broker: leg1.broker || 'Other',
            brokerAccountId: leg1.brokerAccountId || '',
          });
          setTagsInput(cleanTags.join(', ').replace(/#/g, ''));
          setManualBrokerageText(initialManualBrokerage.toString());
          setManualTaxesText(initialManualTaxes.toString());

          const guessedSz = guessLotSize(leg1.symbol || leg2.symbol);
          setLotSizeInput(guessedSz.toString());
          setLotsInput((leg1.qty / guessedSz).toString());
          const match = (leg1.symbol || leg2.symbol).toUpperCase().match(/^(NIFTY|BANKNIFTY|FINNIFTY|MIDCPNIFTY|SENSEX|BANKEX)/);
          if (match) {
        ...
        // Single trade edit
        setEditingLeg1Id(existing.id);
        setEditingLeg2Id(null);
        setIsMultiLeg(false);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isMultiLeg) {
      ...
      if (editingLeg1Id && editingLeg2Id) {
        if (!window.confirm('Are you sure you want to update both hedge spread legs?')) return;
        editTrade(editingLeg1Id, finalTradeData);
        editTrade(editingLeg2Id, finalTrade2Data);
      } else if (editTradeId) {
        if (!window.confirm('Are you sure you want to update this trade into a hedge spread?')) return;
    ...
    <div className="modal-overlay">
      <div className="modal-content glass-card animate-fade-in" style={{ padding: 0, maxHeight: '90vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div className="modal-header">
          <h2>{editTradeId ? (isMultiLeg ? 'Edit Hedged Spread (2 Legs)' : 'Edit Options/Equity Log') : (isMultiLeg ? 'Log Hedged Spread (2 Legs)' : 'Log Options/Equity Trade')}</h2>
          ...
          <div className="modal-body" style={{ overflowY: 'auto', flexGrow: 1, padding: '20px' }}>
            ...
            {/* Grid 1: Basic Metadata */}
                ...
                {formData.segment === 'F&O' && (
                  <>
                    {/* Spread Mode Toggle */}
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(59, 130, 246, 0.05)', padding: '8px 12px', borderRadius: '8px', border: '1px solid rgba(59, 130, 246, 0.15)', marginBottom: '16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        />
                        <label htmlFor="isMultiLeg" style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-main)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>
                          <span>🛡️ Hedge Mode</span>
                          <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 400 }}>(Spread / 2-Leg Trade)</span>
                        </label>
                      </div>

                      {isMultiLeg && editingLeg1Id && editingLeg2Id && (
                        <span style={{ fontSize: '0.72rem', color: 'var(--color-win)', background: 'var(--color-win-bg)', border: '1px solid var(--color-win-border)', padding: '2px 8px', borderRadius: '12px', fontWeight: 600 }}>
                          ✓ Paired Spread Linked (Leg 1 + Leg 2)
                        </span>
                      )}
                    </div>

                    {/* Quick Options Header / Presets */}
                    <div 
                      ...
                      style={{ 
                        ...
                        borderRadius: '8px',
                        display: 'grid',
                        gridTemplateColumns: !isMultiLeg ? 'repeat(3, 1fr)' : 'repeat(3, 1fr)',
                        gap: '16px'
                      }}
                    >
                      <div className="form-group" style={{ marginBottom: 0 }}>
                        <label className="form-label" style={{ color: 'var(--primary)' }}>Underlying Index</label>
                        <select
